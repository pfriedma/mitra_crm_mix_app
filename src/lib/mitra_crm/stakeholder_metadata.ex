defmodule MitraCrm.StakeholderMetadata do
    ...
    #@enforce_keys [:name]
    defstruct [:uuid, :updated_date, crm_uids: [], shared: false, persistance_type: :json, persistance_uri: ""] 
    ...
    defimpl Poison.Decoder, for: StakeholderMetadata do
        def decode(%{
            ...
            persistance_uri: persistance_uri,
        }, options) do
            with {:ok, datetime, _} <- DateTime.from_iso8601(date) do
                %StakeholderMetadata{ 
                    ...
                    crm_uids: crm_uids,
                    shared: shared,
                    persistance_type: String.to_existing_atom(persistance_type),
                    persistance_uri: persistance_uri
                }
            else 
                err -> err
    ...
    def new(crm_uid) do
        {:ok, %StakeholderMetadata{uuid: UUID.uuid1(:urn), updated_date: DateTime.utc_now(), crm_uids: [crm_uid], shared: false, persistance_type: :json, persistance_uri: ""}}
    end
